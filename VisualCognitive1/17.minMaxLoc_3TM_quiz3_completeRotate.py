...
best = {}
minLocs, minVals, tmpl_hws, resizes, rots = [], [], [], [], []
MINVAL = 1
...
top10Idx = np.argsort(minVals)[:10]
for i in top10Idx:
    x, y = minLocs[i]
    h, w = tmpl_hws[i]
    rs = resizes[i]
    rot = round(rots[i], 2)
    minV = round(minVals[i], 3)
    title = f"minVal {minV} at {rs} resized, {rot} rotated image"
    cv2.rectangle(dst, (x,y), (x+w, y+h) , (255,0,0), 1)
...
# [Quiz] TM_CCOEFF_NORMED를 사용할 때의 결과를 출력해보자. maxValue를 선택해야 한다.